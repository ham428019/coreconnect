...
const orderNumber = () => `CC-${new Date().getFullYear()}-${Date.now().toString(36).toUpperCase()}${crypto.randomBytes(4).toString("hex").toUpperCase()}`;
const referenceCode = () => crypto.randomBytes(8).toString("hex").toUpperCase();

const randDateIn = (month: number) => {
  const d = new Date(2026, month, 1);
  const lastDay = new Date(2026, month + 1, 0).getDate();
  d.setDate(1 + Math.floor(Math.random() * lastDay));
...
async function main() {
  ...
  if (customers.length === 0 || products.length < 5) {
    console.log("Not enough data");
    return;
  }

  const customersWithAddresses = await Promise.all(
    customers.map(async (c) => ({ ...c, addresses: await prisma.address.findMany({ where: { userId: c.id } }) }))
  );

  let created = 0;
  for (const cust of customersWithAddresses) {
    const addr = cust.addresses[0];
    const perMonth = [2, 3, 4, 5][Math.floor(Math.random() * 4)];

    for (const month of [2, 3]) { // March = 2, April = 3
      for (let i = 0; i < perMonth; i++) {
        ...
        const createdAt = randDateIn(month);
  ...
  console.log(`Created ${created} orders in March and April 2026`);
}
