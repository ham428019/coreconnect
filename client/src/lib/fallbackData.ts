...
export function getFallbackProducts(category?: string, search?: string): Product[] {
  ...
  return filtered.map((p, idx) => ({
    ...
    tags: p.tags,
    productType: p.tags[0],
    keyFeatures: p.shortDesc.split(',').map(item => item.trim()).filter(Boolean),
    compatibility: [],
    useCases: p.tags.filter(tag => ['gaming', 'esports', 'office', 'productivity', 'programming'].includes(tag)),
    colors: [],
